...
function serve (builder, options) {
    ...
    handler: {
      directory: {
        path: function (request) {
          if (!currentBuildDirectory) throw new Error('Expected currentBuildDirectory to be set')
  ...
  server.ext('onRequest', function (request, reply) {
    // If a build is currently running, this will defer until it has finished
    watcher.then(function(directory) {
      currentBuildDirectory = directory
      reply()
    }, function(buildError) {
      currentBuildDirectory = null
      var context = {
