...
function linkRecursivelySync (src, dest, _mkdirp) {
  ...
  if (srcStats.isDirectory()) {
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      linkRecursivelySync(path.join(src, entries[i]), path.join(dest, entries[i]), false)
    }
  } else {
    if (_mkdirp) {
...
function loadBroccolifile (findupOptions) {
  // Require here to avoid cycles
  var readers = require('./readers')
  var builder = require('./builder')

  var broccolifile = findup('Broccolifile.js', findupOptions)
  ...
  if (env !== 'production' && env !== 'development') {
    throw new Error('Only BROCCOLI_ENV=production and BROCCOLI_ENV=development are supported at the moment')
  }

  var factory = {
    makeTree: function () {
      return new readers.Tree(baseDir)
    },
    makeBuilder: function () {
      // This might start depending on baseDir if we stop calling chdir
      return new builder.Builder()
    },
    env: env
  }

  return require(broccolifile)(factory, broccoli)
}
