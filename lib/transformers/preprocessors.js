...
Preprocessor.prototype._processFile = function (srcFilePath, destFilePath, callback) {
  ...
  this.processString(string, function (err, output) {
    if (err) {
      callback(err)
    } else {
...
Preprocessor.prototype.processString = function (string, callback) {
...
CoffeeScriptPreprocessor.prototype.processString = function (string, callback) {
  ...
  var output
  try {
    output = require('coffee-script').compile(string, optionsCopy)
  } catch (err) {
...
ES6TemplatePreprocessor.prototype.processString = function (string, callback) {
  callback(null,
