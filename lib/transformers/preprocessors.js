...
Preprocessor.prototype._processFile = function (srcFilePath, destFilePath, callback) {
  ...
  this.processContents(contents, function (err, output) {
    if (err) {
      callback(err)
    } else {
...
Preprocessor.prototype.processContents = function (contents, callback) {
...
CoffeeScriptPreprocessor.prototype.processContents = function (contents, callback) {
  ...
  var output
  try {
    output = require('coffee-script').compile(contents, optionsCopy)
  } catch (err) {
...
ES6TemplatePreprocessor.prototype.constructor = ES6TemplatePreprocessor
...
ES6TemplatePreprocessor.prototype.processContents = function (contents, callback) {
  callback(null,
    'export default ' + this.compileFunction +
    '("' + jsStringEscape(contents) + '");\n')
}
