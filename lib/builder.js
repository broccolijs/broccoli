...
module.exports = class Builder extends EventEmitter {
      ...
      promise = promise.then(() => {
        ...
        return promiseFinally(
          Promise.resolve()
            ...
            .then(() => {
              console.log('START', nw.nodeInfo.name);
            })
            .then(() => this.emit('beginNode', nw))
            .then(() => nw.build()),
          () => {
            console.log('DONE', nw.nodeInfo.name);
  ...
  cleanup() {
    let promise = new Promise(resolve => {
      ...
      resolve(this._cancelationRequest);
    });

    promise = promiseFinally(promise, () => this.builderTmpDirCleanup());
  ...
  buildHeimdallTree(outputNodeWrapper) {
    ...
    // During cleaning, heimdall isn't structured right and aggregateTime() causes an OOM
    if (this._isCleaning) {
      return;
    }
