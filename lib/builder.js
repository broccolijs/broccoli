...
Builder.prototype.setupTmpDirs = function() {
  ...
  //   02-otherplugin/
  //     ...
  const tmpobj = tmp.dirSync({ prefix: 'broccoli-', unsafeCleanup: true, dir: this.tmpdir });
...
TransformNodeWrapper.prototype.build = function() {
  ...
  return RSVP.resolve(self.callbackObject.build()).then(function() {
    ...
    for (let i = 0; i < self.inputNodeWrappers.length; i++) {
...
TransformNodeWrapper.prototype.toString = function() {
  ...
  if (this.inputNodeWrappers) {
    ...
    hint +=
      ' inputNodeWrappers:[' +
      this.inputNodeWrappers.map(function(nw) {
        return nw.id;
      }) +
      ']';
  }
...
NodeWrapper.prototype.toJSON = function() {
  return undefinedToNull({
    ...
    label: this.label,
    inputNodeWrappers: this.inputNodeWrappers.map(function(nw) {
      return nw.id;
    }),
    cachePath: this.cachePath,
    outputPath: this.outputPath,
