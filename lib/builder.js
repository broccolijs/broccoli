...
Builder.prototype.build = function() {
  ...
  this.nodeWrappers.forEach(function(nw) {
    ...
    function cleanOutputDirectoryIfNecessary() {
      // It'd be so much nicer to have a TransformNodeWrapper subclass that
      // implements TransformNodeWrapper::build and does the cleaning for us
...
Builder.prototype.makeNodeWrapper = function(node, _stack) {
  ...
  if (node == null || !node.__broccoliGetInfo__) {
    ...
    var messageSuffix = (_stack.length > 0) ?
      ...
      : '\nused as output node'
    throw new InvalidNodeError(message + messageSuffix)
  }
...
Builder.prototype.getNodeInfo = function(node) {
  ...
  // feature set (`this.features`) rather than the smaller feature set we'll
  // be mimicking to interact with this plugin. This is a fairly arbitrary
  // choice, but it's easier to implement, and it usually won't make a
  // difference because the Plugin class won't care about features it doesn't
  // know about.
  ...
  // 1. Use prototypal inheritance to avoid mutating other people's objects
