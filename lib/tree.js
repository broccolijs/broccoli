...
function Tree (injector, baseDirectory) {
...
Tree.prototype.addBower = function () {
  this.addTrees(bowerTrees(this.injector))
  return this
}
...
Tree.prototype.friendlyName = function () {
  return 'Tree ' + this.baseDirectory
}

Tree.prototype.read = function () {
  var self = this
  var destDir = this.injector.get(TmpDirManager).makeTmpDir('tree')
  if (this._sources.length === 0) throw new Error('No tree map for ' + this)
  var dir = this.transformers.length === 0 ? destDir : this.injector.get(TmpDirManager).makeTmpDir('tree')
    ...
    return self._sources.reduce(function (promise, sourceItem) {
      return promise.then(function () {
        if (Array.isArray(sourceItem)) { // mapping
          ...
          return sourceItem.withTimer(function () {
            return sourceItem.read()
          }).then(function (treeDir) {
            helpers.linkRecursivelySync(treeDir, dir)
          })
        }
      })
    }, RSVP.resolve())
  ...
  function applyTransformers () {
    return self.transformers.reduce(function (promise, transformer, index) {
      return promise.then(function () {
        var previousDir = dir
        dir = (index === self.transformers.length - 1) ? destDir : self.injector.get(TmpDirManager).makeTmpDir('tree')
        return transformer.withTimer(function () {
...
Tree._fromDirectory = function (injector, dir) {
  ...
  var tree = new Tree(injector, dir)
...
function bowerTrees (injector) {
  ...
  for (var i = 0; i < directories.length; i++) {
    trees = trees.concat(Tree._fromDirectory(injector, path.join(bowerDir, directories[i])))
  }
  // Pick up files as well; this is for compatibility with EAK's vendor/loader.js
  for (i = 0; i < files.length; i++) {
    trees.push(new Tree(injector, bowerDir).map('/' + files[i], '/' + files[i]))
  }
  return trees
}
