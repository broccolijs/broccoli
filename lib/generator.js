  ...
  function preprocessorsForFile (allPreprocessors, filePath) {
    ...
    while (allPreprocessors.length > 0) {
      ...
      if (preprocessor != null) {
        preprocessorsForFile.push(preprocessor)
        filePath = destPath
      } else {
    ...
    if (preprocessorsForFile.length === 0) {
      preprocessorsForFile.push(new preprocessors.CopyPreprocessor)
    }
    return preprocessorsForFile
  }
