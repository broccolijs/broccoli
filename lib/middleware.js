...
module.exports = function getMiddleware(watcher, options) {
  ...
  return function broccoliMiddleware(request, response, next) {
    watcher.currentBuild.then(function() {
      ...
      // We can *almost*  guarantee that this is a Builder.BuildError object,
      // but not quite, because beginBuild/endBuild event handlers can cause
      // other errors to be caught here. Maybe we should fix that.
      var context = {
        message: buildError.message,
        stack: buildError.stack,
        liveReloadPath: options.liveReloadPath
      }
      if (buildError instanceof watcher.builder.constructor.BuildError) {
        // buildError.message is augmented for easy terminal output; for
        // structured output in the browser we use originalMessage
        context.message = buildError.broccoliPayload.originalMessage
        context.payload = buildError.broccoliPayload
      }
