...
module.exports = class SourceNodeWrapper extends NodeWrapper {
  ...
  reset() {
    this.buildState = {};
    this.outputWrapper.settle();
  }

  revise() {
    // Set revised flag to indicate to build() whether to revise this.outputWrapper
    // Don't revise this.outputWrapper here as revise() is called by the watcher, but reset() is called before build()
    this._revised = true;
  }
