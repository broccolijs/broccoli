...
module.exports = class TransformNodeWrapper extends NodeWrapper {
  setup(features) {
    ...
    this.outputWrapper = new DirectoryWrapper(this.outputPath);
    this.originalNode.output = this.outputWrapper;
  }

  shouldBuild() {
    for (let inputNodeWrapper of this.inputNodeWrappers) {
      if (inputNodeWrapper.outputWrapper.hasChanges()) {
        return true;
      }
    }
  ...
  build() {
    ...
    return new Promise(resolve => {
      ...
      if (this.nodeInfo.sideEffectFree && !this.shouldBuild()) {
        resolve();
        return;
      }
