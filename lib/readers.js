...
Package.prototype.statsHash = function () {
  ...
  var paths
  if (this.options.main) {
    paths = this.options.main.map(function (relativePath) {
      return path.join(self.baseDirectory, relativePath)
    })
  } else {
    paths = [this.getAssetDirectory()]
  }
  return helpers.hashStrings(paths.map(function (fullPath) {
    return helpers.hashTree(fullPath)
  }))
}
