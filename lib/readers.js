...
PackageReader.prototype.read = function (destDir, callback) {
  ...
  async.eachSeries(this.packages, function (package, packageCallback) {
    ...
    if (package.treeTransform == null) {
      ...
      package.treeTransform._setup(self.setupOptions)
      package.treeTransform.run(intermediateDir, destDir, function (err) {
        packageCallback(err)
      })
      // We should perhaps have a target dir and then link from there, so the
      // package's treeTransform doesn't have to support overwriting
    }
  }, function (err) {
    callback(err)
  })
